...
public class WeightImpl implements Weight, TextParsable {
	...
	public static class WKernel implements Kernel {
		...
		public Kernel optimize() {
			if (this.optimizer == null) System.out.println("WARNING: weight has no optimizer");
	...
	private Matrix evaluate(int time, MatrixStack inputs, Matrix bias) {
		...
		if (!Kernel.GLOBAL_BIAS) {assert (bias == null || bias == this.bias(time));}
	...
	private MatrixStack dValue(int time, MatrixStack prevOutputs, Matrix thisError) {
		...
		for (int d = 0; d < depth; d++) {
			...
			dValues[d] = newCore(W1(time, d), W2(time, d)).dValue(prevOutput, thisError);
		}
	...
	private MatrixStack dValue(MatrixStack prevOutputs, MatrixStack thisErrors) {
		...
		if (summode) { //Please pay attention to this code line.
			...
			for (int t = 0; t < time(); t++) {
				MatrixStack ds = dValue(t, prevOutputs, thisErrors.get(t));
				dValues[t] = ds.get(0);
	 ...
	 * @return weight.
	 */
	private static WeightImpl create0(Size sizeW1, Size sizeW2, Size biasSize, NeuronValue hint, boolean bilinear) {
	...
	public static WeightImpl create(Size prevSize, Size size, NeuronValue hint, boolean bilinear) {
		...
		return create0(sizeW1, sizeW2, new Size(size.width, size.height, prevSize.depth, size.depth), hint, bilinear);
	}
	 ...
	 * @return weight.
	 */
	static WeightImpl create(Size prevSize, Size size, NeuronValue hint) {
		return create(prevSize, size, hint, Kernel.BILINEAR);
	}
...
class WCore implements Cloneable, Serializable {
	...
	/**
	 * Calculating gradient of the current first weight.
	 * The three gradient methods {@link #dW1(Matrix, Matrix, Function)}, {@link #dW2(Matrix, Matrix, Function)}, and {@link #dValue(Matrix, Matrix, Matrix, Function)} are the most important in training matrix neural network.
	 * @param prevOutput previous output.
	 * @param thisError current error.
	 * @return gradient of the current first weight.
	 */
...
class WCore1 extends WCore {
	...
	@Override
	Matrix dValue(Matrix prevOutput, Matrix thisError) {
		return this.W1.transpose().multiply(thisError);
	}
...
class WCore2 extends WCore {
	...
	@Override
	Matrix dValue(Matrix prevOutput, Matrix thisError) {
		return thisError.multiply(this.W2.transpose());
	}
