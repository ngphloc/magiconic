...
public class Swarm extends VGGExt {
	...
	protected boolean initialize(net.ea.ann.mane.MatrixLayerAbstract.LayerSpec[] layerSpecs, boolean dual) {
		...
		if (particlesCount > 1) {
			for (int i = 0; i < particlesCount; i++) {
				...
				if (particle instanceof NetworkAbstract) {
					try {
						((NetworkAbstract)particle).setConfig(getConfig());
					} catch (Throwable e) {Util.trace(e);}
				}
				if (particle instanceof Swarm) {
					try {
						((Swarm)particle).paramSetParticlesCount(0);
					} catch (Throwable e) {Util.trace(e);}
				}
	...
	public Swarm paramSetParticlesCount(int particles) {
		particles = particles < 0 ? PARTICLES_COUNT_DEFAULT : particles;
