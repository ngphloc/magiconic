...
/**
 * This class represents a zoom out as simple convolution filter in 2D space.
 * 
 * @author Loc Nguyen
 * @version 1.0
 *
 */
public class ZoomOutFilter2D extends AbstractFilter2D {
	 ...
	 */
	protected ZoomOutFilter2D(int width, int height) {
		super();
		this.width = width;
		this.height = height;
	}
	 ...
	 */
	public static ZoomOutFilter2D create(Size size) {
		if (size.width < 1) size.width = 1;
		if (size.height < 1) size.height = 1;
		
		return new ZoomOutFilter2D(size.width, size.height);
	}
