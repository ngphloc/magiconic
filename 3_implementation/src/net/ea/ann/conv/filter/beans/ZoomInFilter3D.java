...
import net.ea.ann.conv.filter.AbstractDeconvFilter3D;
import net.ea.ann.core.value.NeuronValue;
import net.ea.ann.raster.Size;

/**
 * This class represents a zoom in as simple deconvolution filter in 3D space.
 * 
 * @author Loc Nguyen
 * @version 1.0
 *
 */
public class ZoomInFilter3D extends AbstractDeconvFilter3D {
	 ...
	 */
	protected ZoomInFilter3D(int width, int height, int depth) {
		super();
		this.width = width;
		this.height = height;
		this.depth = depth;
	 ...
	 */
	public static ZoomInFilter3D create(Size size) {
		if (size.width < 1) size.width = 1;
		if (size.height < 1) size.height = 1;
		if (size.depth < 1) size.depth = 1;
		
		return new ZoomInFilter3D(size.width, size.height, size.depth);
	}
