...
/**
 * This class represents the default deconvolutional filter based on a convolutional filter in 3D space.
 * 
 * @author Loc Nguyen
 * @version 1.0
 *
 */
public class DeconvConvFilter3DImpl extends AbstractDeconvFilter3D implements DeconvConvFilter3D {
	 ...
	 */
	protected ProductFilter3D convFilter = null;
	 ...
	 */
	protected DeconvConvFilter3DImpl(ProductFilter3D convFilter) {
		super();
	...
	public NeuronValue apply(int x, int y, int z, ConvLayerSingle3D layer, int nextX, int nextY, int nextZ, ConvLayerSingle3D nextLayer) {
		...
		if (nextLayer == null) return apply(x, y, z, layer);
		...
		Cube nextRegion = new Cube(x*filterStrideWidth, y*filterStrideHeight, z*filterStrideDepth,
		...
		if (nextRegion.x < 0 || nextRegion.x >= nextWidth || nextRegion.y < 0 || nextRegion.y >= nextHeight || nextRegion.z < 0 || nextRegion.z >= nextDepth)
			return null;
		if (!nextRegion.contains(nextX, nextY, nextZ))
			return null;
			
		NeuronValue nextResult = nextLayer.newNeuronValue().zero();
		NeuronValue value0 = layer.get(x, y, z).getValue();
		int kernelX = -1;
		int kernelY = -1;
		int kernelZ = -1;
		for (int i = 0; i < kernelDepth; i++) {
			...
			for (int j = 0; j < kernelHeight; j++) {
				...
				for (int k = 0; k < kernelWidth; k++) {
					...
					NeuronValue value = nextLayer.get(X, Y, Z).getValue();
					value = value == null ? value0 : value; //Smoothing trick.
					nextResult = nextResult.add(value.multiply(convFilter.kernel[i][j][k]));
				}
			}
		}
		nextResult = nextResult.multiply(convFilter.weight);
		
		NeuronValue xyzWeight = null;
		if (kernelX >= 0 && kernelY >= 0 && kernelZ >= 0)
			xyzWeight = convFilter.kernel[kernelZ][kernelY][kernelX];
		
		NeuronValue result = layer.get(x, y, z).getValue();
		if (xyzWeight != null && xyzWeight.canInvert()) {
			result = result.subtract(nextResult);
			result = result.divide(xyzWeight.multiply(convFilter.weight));
		}
	 ...
	 */
	public static DeconvConvFilter3D create(ProductFilter3D convFilter) {
		if (convFilter == null)
			return null;
		else
			return new DeconvConvFilter3DImpl(convFilter);
	}
