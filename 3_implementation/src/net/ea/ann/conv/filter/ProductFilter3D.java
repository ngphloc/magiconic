...
/**
 * This class represents a product filter in 3D space.
 * 
 * @author Loc Nguyen
 * @version 1.0
 *
 */
public class ProductFilter3D extends AbstractFilter3D implements TextParsable {
	 ...
	 */
	protected ProductFilter3D(NeuronValue[][][] kernel, NeuronValue weight) {
		super();
		this.kernel = kernel;
		this.weight = weight;
		
		this.strideWidth = kernel[0][0].length;
		this.strideHeight = kernel[0].length;
		this.strideDepth = kernel.length;
	}
		...
		for (int i = 0; i < kernelDepth; i++) {
			for (int j = 0; j < kernelHeight; j++) {
				for (int k = 0; k < kernelWidth; k++) {
					...
					result = result.add(value.multiply(kernel[i][j][k]));
				}
			}
		}
		...
		for (int i = 0; i < depth(); i++) {
			...
			for (int j = 0; j < height(); j++) {
				...
				for (int k = 0; k < width(); k++) {
					...
					if (kernel[i][j][k] instanceof TextParsable)
						buffer.append(((TextParsable)kernel[i][j][k]).toText());
					else
						buffer.append(kernel[i][j][k]);
	 ...
	 */
	public static ProductFilter3D create(NeuronValue[][][] kernel, NeuronValue weight) {
		if (kernel == null || weight == null) return null;
		
		return new ProductFilter3D(kernel, weight);
	}
	 ...
	 */
	public static ProductFilter3D create(double[][][] kernel, double weight, NeuronValueCreator creator) {
		if (kernel == null) return null;
		
		int depth = kernel.length;
		int height = kernel[0].length;
		int width = kernel[0][0].length;
		NeuronValue[][][] newKernel = new NeuronValue[depth][height][width];
		NeuronValue source = creator.newNeuronValue();
		for (int i = 0; i < depth; i++) {
			for (int j = 0; j < height; j++) {
				for (int k = 0; k < width; k++) newKernel[i][j][k] = source.valueOf(kernel[i][j][k]);
			}
		}
		
		NeuronValue newWeight = source.valueOf(weight);
		return new ProductFilter3D(newKernel, newWeight);
	}
	 ...
	 */
	public static ProductFilter3D create(Size size, NeuronValueCreator creator) {
		if (size.width < 1) size.width = 1;
		if (size.height < 1) size.height = 1;
		if (size.depth < 1) size.depth = 1;
		
		NeuronValue source = creator.newNeuronValue();
		NeuronValue[][][] kernel = new NeuronValue[size.depth][size.height][size.width];
		for (int i = 0; i < size.depth; i++) {
			for (int j = 0; j < size.height; j++) {
				for (int k = 0; k < size.width; k++) kernel[i][j][k] = source.zero();
			}
		}
